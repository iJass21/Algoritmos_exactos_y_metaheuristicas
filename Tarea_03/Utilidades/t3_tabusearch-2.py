...
def run_ga(func, dim, bounds, pop_size, max_generations, mutation_rate=0.05, crossover_rate=0.3):
    ...
    population = scatter_search(population, func, bounds)
...
def save_results_to_txt(func_name, results, initial_parameters, best_result, best_iteration, avg_fitness, avg_cpu_time):
    ...
    with open(filename, 'a') as file:
        ...
        for result in results:
            best_solution_str = ', '.join(map(str, result[0]))
        ...
        best_solution_str = ', '.join(map(str, best_result[0]))
...
def tabu_search(individual, func, bounds, max_iterations=50, tabu_size=15, neighborhood_size=25):
    best = individual.copy()
    best_fitness = func(best)
    tabu_list = [best.tolist()]
    for _ in range(max_iterations):
        neighbors = []
        for _ in range(neighborhood_size):
            neighbor = best + np.random.uniform(-0.1, 0.1, size=best.shape)
            neighbor = np.clip(neighbor, bounds[0], bounds[1])
            neighbors.append(neighbor)
        neighbors_fitness = [func(neigh) for neigh in neighbors]
        for neigh, neigh_fitness in zip(neighbors, neighbors_fitness):
            if neigh.tolist() not in tabu_list and neigh_fitness < best_fitness:
                best = neigh.copy()
                best_fitness = neigh_fitness
                if len(tabu_list) >= tabu_size:
                    tabu_list.pop(0)
                tabu_list.append(neigh.tolist())
    return best

def scatter_search(population, func, bounds):
    return np.array([tabu_search(ind, func, bounds) for ind in population])
