...
def tournament_selection(population, fitness, k=3):
    selected = []
    pop_size = len(population)  # Aquí se corrige el error
...
def tabu_search(individual, func, bounds, max_iter=100, tabu_size=10):
    ...
    for _ in range(max_iter):
        neighborhood = [mutate(best_individual, 1.0, bounds) for _ in range(50)]
        ...
        if best_neighbor_fitness < best_fitness and not any(np.array_equal(best_neighbor, t) for t in tabu_list):
...
def scatter_search(population, func, bounds, max_iter=100, tabu_size=10):
    for i in range(len(population)):
        population[i] = tabu_search(population[i], func, bounds, max_iter, tabu_size)
    return population
...
mutation_rate = 0.03 #valor inicial: 0.01. 0.05
